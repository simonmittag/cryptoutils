/*
 * PropertyBasedCipherKeyWrapperTest
 */
package com.simonmittag.asymmetric;

import com.simonmittag.cryptoutils.asymmetric.AsymmetricKeyRSACipher;
import com.simonmittag.cryptoutils.asymmetric.PropertyBasedCipherKeyWrapper;
import junit.framework.TestCase;

/**
 * @author simonmittag
 * @since <version>
 */
public class PropertyBasedCipherKeyWrapperTest extends TestCase{

    private PropertyBasedCipherKeyWrapper propertyBasedCipherKeyWrapper;
    public static final String TEST_PUBLIC_KEY = "";
    public static final String TEST_PRIVATE_KEY="";
    public void setUp(){
        System.setProperty(PropertyBasedCipherKeyWrapper.ASYMMETRIC_PUBLIC_KEY,
                TEST_PUBLIC_KEY);
        System.setProperty(PropertyBasedCipherKeyWrapper.ASYMMETRIC_PRIVATE_KEY,
                TEST_PRIVATE_KEY);
        propertyBasedCipherKeyWrapper = new PropertyBasedCipherKeyWrapper(new AsymmetricKeyRSACipher());

    }
    public void testEncryptDecrypt() {
        String encryptedString = propertyBasedCipherKeyWrapper.encrypt("Hello World");
        assertEquals("Hello World",propertyBasedCipherKeyWrapper.decrypt(encryptedString));

    }
}
